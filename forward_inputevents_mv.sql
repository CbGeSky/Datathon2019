...
SELECT
  forw_in_mv.subject_id, forw_in_mv.hadm_id, forw_in_mv.icustay_id
...
( -- begin query that extracts the data
  ...
  LEFT JOIN inputevents_mv le
    ON le.subject_id = ie.subject_id AND le.hadm_id = ie.hadm_id AND le.icustay_id = ie.icustay_id
    AND ( le.starttime > ie.charttime_first ) AND (le.endtime < ie.charttime_na_high)
    AND le.itemid in
    (
...
GROUP BY forw_in_mv.subject_id, forw_in_mv.hadm_id, forw_in_mv.icustay_id
ORDER BY forw_in_mv.subject_id, forw_in_mv.hadm_id, forw_in_mv.icustay_id;
